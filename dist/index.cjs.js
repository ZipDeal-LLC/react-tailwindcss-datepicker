...
const Days = ({ calendarData, onClickPreviousDays, onClickDay, onClickNextDays }) => {
    ...
    const isDisabled = React.useCallback((item, type = "") => {
        ...
        return configs?.past && formatDate(dayjs__default["default"](itemDate)) > formatDate(dayjs__default["default"]());
...
const Week = () => {
    const { i18n } = React.useContext(DatepickerContext);
    loadLanguageModule(i18n);
    return (React__default["default"].createElement("div", { className: "grid grid-cols-7 border-b border-gray-300 py-2" }, [0, 1, 2, 3, 4, 5, 6].map((item, index) => (React__default["default"].createElement("div", { key: index, className: "tracking-wide text-gray-500 text-center" }, ucFirst(shortString(dayjs__default["default"](`2022-11-${6 + item}`)
        .locale(i18n)
        .format("ddd"))))))));
};
...
const Footer = () => {
    ...
    const valueString = React.useMemo(() => {
        if (!period?.start || !period?.end)
            return "";
        return `${dateStringFormat ? dayjs__default["default"](period?.start).format(dateStringFormat) : period?.start} ${separator} ${dateStringFormat ? dayjs__default["default"](period?.end).format(dateStringFormat) : period?.end}`;
    ...
    return (React__default["default"].createElement("div", { className: "flex items-center justify-end pb-2.5 pt-3 border-t border-gray-300 gap-x-10" },
        ...
        React__default["default"].createElement("div", { className: "w-full md:w-auto flex items-center justify-center space-x-3" },
            ...
            React__default["default"].createElement(PrimaryButton, { onClick: () => {
                    if (period.start && period.end) {
                        changeDatepickerValue({
                            startDate: dayjs__default["default"](period.start).format(dateValueFormat),
                            endDate: dayjs__default["default"](period.end).format(dateValueFormat)
                        });
                        hideDatepicker();
                    }
...
const Input = ({ inputClassName }) => {
    ...
    const handleInputChange = React.useCallback((e) => {
        ...
        if (start.length === 10 &&
            ...
            changeDatepickerValue({
                startDate: dayjs__default["default"](start).format(dateValueFormat),
                endDate: dayjs__default["default"](end).format(dateValueFormat)
            });
            changeDayHover(dayjs__default["default"](end).add(-1, "day").format(dateValueFormat));
            hideDatepicker();
            if (input) {
                input.blur();
            }
...
const ItemTemplate = React__default["default"].memo((props) => {
    ...
    const chosePeriod = React.useCallback((item) => {
        const start = dateValueFormat ? dayjs__default["default"](item.start).format(dateValueFormat) : item.start;
        const end = dateValueFormat ? dayjs__default["default"](item.end).format(dateValueFormat) : item.end;
        if (dayHover) {
            changeDayHover(null);
        }
        if (period.start || period.end) {
        ...
        if (!configs?.past)
            updateFirstDate(dayjs__default["default"](start));
        hideDatepicker();
    }, [
        dateValueFormat,
        dayHover,
...
const Datepicker = ({ primaryColor = "blue", value = null, onChange, useRange = true, showFooter = false, showShortcuts = false, configs = null, asSingle = false, placeholder = null, separator = "~", i18n = "en", dateValueFormat = "", dateStringFormat = "", inputClassName = "" }) => {
    ...
    const initialValues = React.useMemo(() => {
        const start = value?.startDate && dayjs__default["default"](value?.startDate).isValid()
            ? dayjs__default["default"](value?.startDate)
            : dayjs__default["default"]();
        const end = value?.endDate && dayjs__default["default"](value?.endDate).isValid() ? dayjs__default["default"](value?.endDate) : dayjs__default["default"]();
        if (configs?.past) {
            return {
