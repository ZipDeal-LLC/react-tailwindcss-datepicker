...
const Week = () => {
    const { i18n } = useContext(DatepickerContext);
    loadLanguageModule(i18n);
    return (React.createElement("div", { className: "grid grid-cols-7 border-b border-gray-300 py-2" }, [0, 1, 2, 3, 4, 5, 6].map((item, index) => (React.createElement("div", { key: index, className: "tracking-wide text-gray-500 text-center" }, ucFirst(shortString(dayjs(`2022-11-${6 + item}`)
        .locale(i18n)
        .format("ddd"))))))));
};
...
const Footer = () => {
    ...
    const valueString = useMemo(() => {
        if (!period?.start || !period?.end)
            return "";
        return `${dateStringFormat ? dayjs(period?.start).format(dateStringFormat) : period?.start} ${separator} ${dateStringFormat ? dayjs(period?.end).format(dateStringFormat) : period?.end}`;
...
const Datepicker = ({ primaryColor = "blue", value = null, onChange, useRange = true, showFooter = false, showShortcuts = false, configs = null, asSingle = false, placeholder = null, separator = "~", i18n = "en", dateValueFormat = "", dateStringFormat = "", inputClassName = "" }) => {
    ...
    const initialValues = useMemo(() => {
        const start = value?.startDate && dayjs(value?.startDate).isValid()
            ? dayjs(value?.startDate)
            : dayjs();
        const end = value?.endDate && dayjs(value?.endDate).isValid() ? dayjs(value?.endDate) : dayjs();
        if (configs?.past) {
            return {
